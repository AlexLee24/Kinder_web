            ...
            dates.forEach(d => {
                ...
                if (log) {
                    ...
                    const priorityBadge = (() => {
                        ...
                        const prog = (log.program || '').trim();
                        const display = prog ? `${p} - ${prog}` : p;
...
function openLogModalEdit(uniqueTargetId, dateStr) {
    ...
    if (prioritySelect) {
        ...
        let programText = (log && log.program) ? log.program.trim() : '';
        if (!programText && rawPriority.includes('-')) {
            // fallback: legacy combined string in priority field
            const partsPri = rawPriority.split('-');
            basePriority = (partsPri.shift() || 'Normal').trim();
            programText = partsPri.join('-').trim();
        } else if (!programText && targetInfo && targetInfo.program) {
...
function saveObservationLog(event) {
    ...
    const priority = basePriority || null;   // obs.logs.priority must be one of Urgent/High/Normal/Filler
