...
const ICONS = {
    ...
    delete: '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>'
};
...
function loadUsersData() {
    ...
    userRows.forEach(row => {
        ...
        const nameEl  = cells[0].querySelector('.user-cell-name');
        const emailEl = cells[0].querySelector('.user-cell-email');
        allUsers.push({
            element: row,
...
async function handleGroupRequest(email, groupName, action) {
    if (action === 'reject' && !confirm('Are you sure you want to reject this request?')) {
        return;
    }
    
    try {
        const response = await fetch(`/admin/group-requests/${action}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ email: email, group_name: groupName })
        });
