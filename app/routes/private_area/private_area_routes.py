...
# Physical location of the JPGs generated by /generate_plot (astronomy_tools blueprint) —
# app/routes/planners/ov_plot, a sibling directory under app/routes/.
_TRIGGER_OV_PLOT_DIR = os.path.abspath(
...
def daily_trigger_send_message():
    ...
    # Resolve the plot_url (e.g. "/ov_plot/observing_tracks_<hex>.jpg") to a real
    # file path, staying strictly inside the ov_plot directory (no traversal).
    image_path = None
    if plot_url:
        filename = os.path.basename(plot_url)
        if re.match(r'^observing_tracks_[0-9a-fA-F]+\.jpg$', filename):
            candidate = os.path.abspath(os.path.join(_TRIGGER_OV_PLOT_DIR, filename))
            if candidate.startswith(_TRIGGER_OV_PLOT_DIR + os.sep) and os.path.isfile(candidate):
                image_path = candidate

    try:
        from modules.trigger_send import send_to_slack, mark_sent
        send_to_slack(script, image_path=image_path)
...
def _log_triggered_targets(telescope, targets, sent_by):
    ...
    from modules.trigger_script import exposure_time as _auto_exposure_time
