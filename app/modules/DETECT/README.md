...
* `modules/detect_pipeline.py` calls `function.run_detect` after each TNS import
  (hourly), for the object page's Run button, and daily for every Follow-up object.
* `modules/ext_M_calculator.py` is a shim over `function.module.calculator`, so
...
Runtime needs `DETECT_DATA_DIR` (set by `main.py` to `app/modules/DETECT/data`,
