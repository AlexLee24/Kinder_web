...
class _Run:
    ...
    def __enter__(self):
        ...
        try:
            from app.services.jobs import job_status
...
def _pin_db_env() -> None:
    ...
    app hard-codes its database name (app.db.DB_NAME) and several modules
    re-load kinder.env with override=True, so pin the name right before each run."""
    from app.db import DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME
