...
Writes to app/log/YYYY-MM-DD.log (retains last 7 days) and echoes every record to the
terminal. Redirects sys.stdout so all print() calls are captured too.
"""
import io
import logging
import os
...
def setup_logging(log_dir: str) -> None:
    ...
    root.handlers = [h for h in root.handlers
                     if not isinstance(h, DailyFileHandler) and getattr(h, 'name', '') != 'kinder-console']
    root.addHandler(handler)

    # Echo everything to the terminal as well (the real stderr, so it still works after
    # sys.stdout is redirected below). gunicorn forwards stderr, so this shows up in both
    # `python run.py` and `gunicorn wsgi:app`.
